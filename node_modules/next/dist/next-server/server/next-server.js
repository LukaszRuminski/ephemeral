...
var _checkCustomRoutes = require("../../lib/check-custom-routes");
...
var _loadEnvConfig = require("../../lib/load-env-config");
...
class Server {
    constructor({dir = '.', staticMarkup = false, quiet = false, conf = null, dev = false, customServer = true} = {}) {
        ...
        (0, _loadEnvConfig.loadEnvConfig)(this.dir, dev);
...
// publicRuntimeConfig gets it's default in client/index.js
        ...
        this.renderOpts = {
            ...
            ampOptimizerConfig: (_this$nextConfig$expe = this.nextConfig.experimental.amp) === null || _this$nextConfig$expe === void 0 ? void 0 : _this$nextConfig$expe.optimizer,
...
// It'll be rendered as part of __NEXT_DATA__ on the client side
        ...
        if (compress && this.nextConfig.target === 'server') {
            ...
            // console.log('compress.this', this)
...
// individually in serverless bundles when deployed
    ...
    logError(err) {
        ...
        if (this.quiet) return;// tslint:disable-next-line
        console.error(err);
    }
    
    async handleRequest(req, res, parsedUrl) {// Parse url if parsedUrl not provided
        ...
        const {basePath} = this.nextConfig.experimental;// if basePath is set require it be present
        if (basePath && !req.url.startsWith(basePath)) {
            return this.render404(req, res, parsedUrl);
        } else {// If replace ends up replacing the full url it'll be `undefined`, meaning we have to default it to `/`
    ...
    generateRoutes() {
        ...
        const staticFilesRoute = this.hasStaticDir ? [{// It's very important to keep this route's param optional.
...
// See more: https://github.com/vercel/next.js/issues/2617
            ...
            fn: async (req, res, params, parsedUrl) => {
                const p = (0, _path.join)(this.dir, 'static', ...(params.path || []).map(encodeURIComponent));
        ...
        const fsRoutes = [{
            ...
            fn: async (req, res, params, parsedUrl) => {// The commons folder holds commonschunk files
...
// make sure to 404 for /_next/static itself
                ...
                if (params.path[0] === _constants.CLIENT_STATIC_FILES_RUNTIME || params.path[0] === 'chunks' || params.path[0] === 'css' || params.path[0] === 'media' || params.path[0] === this.buildId) {
            ...
            fn: async (req, res, params, _parsedUrl) => {// Make sure to 404 for /_next/data/ itself and
// we also want to 404 if the buildId isn't correct
                ...
                const pathname = `/${params.path.join('/')}`.replace(/\.json$/, '').replace(/\/index$/, '/');
                const parsedUrl = (0, _url.parse)(pathname, true);
                await this.render(req, res, pathname, {..._parsedUrl.query, _nextDataReq: '1'}, parsedUrl);
                return {finished: true};
            }
        }, {
            match: (0, _router.route)('/_next/:path*'), type: 'route', name: '_next catchall',// This path is needed because `render()` does a check for `/_next` and the calls the routing again
        ...
        if (this.customRoutes) {
            ...
            const updateHeaderValue = (value, params) => {
                ...
                const {parsedDestination} = (0, _router.prepareDestination)(value, params, {});
                if (!parsedDestination.pathname || !parsedDestination.pathname.startsWith('/')) {// the value needs to start with a forward-slash to be compiled
// correctly
                    return (0, _pathToRegexp.compile)(`/${value}`, {validate: false})(params).substr(1);
                }
                return (0, _url.format)(parsedDestination);
            ...
            redirects = this.customRoutes.redirects.map(redirect => {
                ...
                return {
                    ...
                    fn: async (_req, res, params, parsedUrl) => {
                        const {parsedDestination} = (0, _router.prepareDestination)(route.destination, params, parsedUrl.query);
                        const updatedDestination = (0, _url.format)(parsedDestination);
                        res.setHeader('Location', updatedDestination);
                        res.statusCode = (0, _checkCustomRoutes.getRedirectStatus)(route);// Since IE11 doesn't support the 308 header add backwards
// compatibility using refresh header
            ...
            rewrites = this.customRoutes.rewrites.map(rewrite => {
                ...
                return {
                    ...
                    match: route.match,
                    fn: async (req, res, params, parsedUrl) => {
                        const {newUrl, parsedDestination} = (0, _router.prepareDestination)(route.destination, params, parsedUrl.query, true);// external rewrite, proxy it
                        if (parsedDestination.protocol) {
                            const target = (0, _url.format)(parsedDestination);
                            const proxy = new _httpProxy.default({target, changeOrigin: true, ignorePath: true});
                            proxy.web(req, res);
                            proxy.on('error', err => {
                        ...
                        return {finished: false, pathname: newUrl, query: parsedDestination.query};
                    }
                };
            });
        }
        const catchAllRoute = {
            ...
            fn: async (req, res, params, parsedUrl) => {
                ...
                if ((params === null || params === void 0 ? void 0 : (_params$path = params.path) === null || _params$path === void 0 ? void 0 : _params$path[0]) === 'api') {
...
// or else it won't be in the manifest yet
        ...
        await (0, _apiUtils.apiResolver)(req, res, query, pageModule, this.renderOpts.previewProps, this.onErrorMiddleware);
        return true;
    }
    
    generatePublicRoutes() {
        const publicFiles = new Set((0, _recursiveReaddirSync.recursiveReadDirSync)(this.publicDir).map(p => p.replace(/\\/g, '/')));
        return [{
            match: (0, _router.route)('/:path*'),
            ...
            fn: async (req, res, params, parsedUrl) => {
                ...
                if (publicFiles.has(path)) {
                    await this.serveStatic(req, res,// we need to re-encode it since send decodes it
                        (0, _path.join)(this.publicDir, ...pathParts.map(encodeURIComponent)), parsedUrl);
                    return {finished: true};
                }
                return {finished: false};
            }
    ...
    getDynamicRoutes() {
        return (0, _utils.getSortedRoutes)(Object.keys(this.pagesManifest)).filter(_utils.isDynamicRoute).map(page => ({
            page,
    ...
    async sendHTML(req, res, html) {
        ...
        return (0, _sendHtml.sendHTML)(req, res, html, {generateEtags, poweredByHeader});
    }
...
// call render so we special case this to prevent an infinite loop
    ...
    async findPageComponents(pathname, query = {}, params = null) {
        const paths = [// try serving a static AMP version first
            query.amp ? (0, _normalizePagePath.normalizePagePath)(pathname) + '.amp' : null, pathname].filter(Boolean);
        for (const pagePath of paths) {
            try {
                const components = await (0, _loadComponents.loadComponents)(this.distDir, this.buildId, pagePath, !this.renderOpts.dev && this._isLikeServerless);
                return {
                    components,
                    query: {
    ...
    async getStaticPaths(pathname) {// we lazy load the staticPaths to prevent the user
// from waiting on them for the page to load in dev mode
        let staticPaths;
        let hasStaticFallback = false;
        if (!this.renderOpts.dev) {// `staticPaths` is intentionally set to `undefined` as it should've
// been caught when checking disk data.
            staticPaths = undefined;// Read whether or not fallback should exist from the manifest.
            hasStaticFallback = typeof this.getPrerenderManifest().dynamicRoutes[pathname].fallback === 'string';
        } else {
    ...
    async renderToHTMLWithComponents(req, res, pathname, {components, query}, opts) {// we need to ensure the status code if /404 is visited directly
        if (pathname === '/404') {
            res.statusCode = 404;
        }// handle static page
...
// for direct page visit and /_next/data visit
        ...
        const ssgCacheKey = isPreviewMode || !isSSG ? undefined// Preview mode bypasses the cache
            : `${urlPathname}${query.amp ? '.amp' : ''}`;// Complete the response with cached data if its present
        const cachedData = ssgCacheKey ? await (0, _sprCache.getSprCache)(ssgCacheKey) : undefined;
        if (cachedData) {
            const data = isDataReq ? JSON.stringify(cachedData.pageData) : cachedData.html;
            (0, _sendPayload.sendPayload)(res, data, isDataReq ? 'json' : 'html', !this.renderOpts.dev ? {
                private: isPreviewMode, stateful: false,// GSP response
            ...
            } : undefined);// Stop the request chain here if the data we sent was up-to-date
        ...
        const doRender = maybeCoalesceInvoke(async function () {
            ...
            if (isLikeServerless) {
                ...
                const renderOpts = {...components, ...opts, isDataReq};
        ...
        const {staticPaths, hasStaticFallback} = hasStaticPaths ? await this.getStaticPaths(pathname) : {
...
// getStaticPaths.
            ...
            if (isProduction) {
                html = await (0, _sprCache.getFallback)(pathname);
            ...
            (0, _sendPayload.sendPayload)(res, html, 'html');
        }
        const {isOrigin, value: {html, pageData, sprRevalidate}} = await doRender();
        let resHtml = html;
        if (!(0, _utils2.isResSent)(res) && (isSSG || isDataReq || isServerProps)) {
            (0, _sendPayload.sendPayload)(res, isDataReq ? JSON.stringify(pageData) : html, isDataReq ? 'json' : 'html', !this.renderOpts.dev || isServerProps && !isDataReq ? {
        ...
        if (isOrigin && ssgCacheKey) {
            await (0, _sprCache.setSprCache)(ssgCacheKey, {html: html, pageData}, sprRevalidate);
        }
        return resHtml;
    }
        ...
        try {
            ...
            if (this.dynamicRoutes) {
                for (const dynamicRoute of this.dynamicRoutes) {
                    ...
                    if (result) {
                        try {
                            return await this.renderToHTMLWithComponents(req, res, dynamicRoute.page, result, {
                                ...this.renderOpts,
                                params
                            });
                        } catch (err) {
    ...
    async renderErrorToHTML(err, req, res, _pathname, query = {}) {
        ...
        try {
            try {
                html = await this.renderToHTMLWithComponents(req, res, using404Page ? '/404' : '/_error', result, {
                    ...this.renderOpts,
                    err
                });
            } catch (err) {
    ...
    isServeableUrl(untrustedFileUrl) {// This method mimics what the version of `send` we use does:
...
//    https://github.com/pillarjs/send/blob/de073ed3237ade9ff71c61673a34474b30e5d45b/index.js#L561
        ...
        try {// (1) Decode the URL so we have the proper file name
...
function prepareServerlessUrl(req, query) {
    ...
    req.url = (0, _url.format)({...curUrl, search: undefined, query: {...curUrl.query, ...query}});
}
